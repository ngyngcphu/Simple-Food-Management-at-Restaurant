...
export class FoodTable extends PureComponent {
  ...
  saveEditModal = async (id, editFood) => {
    try {
      ...
      const updatedFoodData = [...this.state.foodData];
  ...
  deleteEditModal = async (id) => {
    try {
      ...
      console.log(response.data);
      this.setState({
        foodData: this.state.foodData.filter((food) => food.id !== id),
        showEditModal: false,
      });
    } catch (error) {
      console.error(error);
    ...
    return (
      <>
        <DetailModal
          ...
          food={
            this.state.foodData.find((x) => x.id === this.state.edittingId) ??
            {}
          }
            ...
            >
              {this.state.lists.map((list, index) => (
                <Button
                  ...
                  variant="primary"
                  onClick={() => {
                    if (this.state.type === list) this.setState({ type: "" });
